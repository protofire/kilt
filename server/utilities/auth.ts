...
export function setCookie(res: Response, { name, data }: userCookieData) {
  ...
  res.setHeader(
    ...
    serialize(name, data, {
      ...
      secure: true,
      domain: "localhost",
      expires: new Date(new Date().getTime() + ms('2 days')) /* ms(process.env.JWT_EXPIRY) ) */,
    })
  );
}
...
export function createJWT(subject: any) {
  ...
  const expiresIn = /* ms(process.env.JWT_EXPIRY) */  ms('2 days');
...
export function getCookieData({ name, cookie }: userData) {
  let data = null;
  try {
