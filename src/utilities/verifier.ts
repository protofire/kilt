...
export const mnemonic: string = process.env.VERIFIER_MNEMONIC ?? ''
...
export async function relationships() {
  ...
  return {
    [KeyRelationship.authentication]: pairs.authentication,
    [KeyRelationship.assertionMethod]: pairs.assertion,
    [KeyRelationship.keyAgreement]: pairs.keyAgreement,
  }
}

export async function getFullDid() {
  const fullDid = await Did.FullDidDetails.fromChainInfo(process.env.VERIFIER_DID_URy as any)
  return fullDid
}

export async function decryptChallenge(
  encryptedChallenge: any,
  encryptionKey: any,
  nonce: any
) {
