...
export function setCookie(res: any, { name, data }: any) {
...
export function createJWT(subject: any) {
...
export function clearCookie(res: any, { name }: any) {
...
export function getCookieData({ name, cookie } : any) {
  let data = null
  try {
...
export async function getDidFromValidSignature({ input, output }: any) {
  ...
  const didDocument: any = await Did.DidResolver.resolveDoc(didUri);
  if (!didDocument) {
